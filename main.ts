...
let lastPaymentMethod: string = "";
let lastPaymentAmount: number = 0;
...
class Student {
  ...
  enroll_course(course: string, fee: number, method: string) {
    if (this.balance >= fee) {
...
class StudentManager {
  ...
  // Method to pay student fees
  async pay_student_fees(studentId: number, amount: number, method: string) {
  ...
  while (true) {
    ...
    switch (choice.choice) {
      ...
      case "Enroll Student":
        ...
        await studentManager.enroll_student(
          course_input.student_id,
          course_input.course,
          tuitionFees[course_input.course]
        );
        break;
      ...
      case "Pay Student Fees":
        let fees_input = await inquirer.prompt([
          ...
          },
          {
            ...
            validate: function (value: string) {
              if (value.trim() !== "" && !isNaN(parseFloat(value))) return true;
